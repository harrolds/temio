...
export default function Huur() {
  ...
  const handleDelete = (id) => { removeItem(id); toastReminder(t(`${formName}.toast.deleted`)); };

  return (
    <SectionCard title={t("pages.huur.title")}>
      ...
      <div className="card-list mt-3">
        {items.length === 0 && <p className="text-muted">{t(`${formName}.noReminders`)}</p>}
        {items.map((item) => (
          <div key={item.id} className="card-item">
            ...
            <div className="mt-2">
              <button type="button" onClick={() => handleEdit(item)}>{t("buttons.bewerken")}</button>
              <button type="button" onClick={() => handleDelete(item.id)}>{t("buttons.verwijderen")}</button>
            </div>
          </div>
        ))}
      </div>
