...
export default function Contracten() {
  ...
  return (
    <div className="rr-page">
      <h2>{t('pages.contracten.title', 'Contracten')}</h2>

      {/* ✅ FORMULIER */}
      <form onSubmit={handleSubmit} className="rr-form-grid">
        <div className="field-group">
          <div className="field">
            <label>{t('pages.contracten.name', 'Naam')}</label>
            <input
              name="naam"
              type="text"
              required
              defaultValue={draft?.naam || ''}
              placeholder={t('pages.contracten.name.placeholder', 'Bijv. Energiecontract')}
            />
          </div>

          <div className="field">
            <label>{t('pages.contracten.datum', 'Vervaldatum')}</label>
            <input
              name="datum"
              type="date"
              required
        ...
        <div className="field actions">
          <button type="submit">{editIndex !== null ? t('common.save', 'Opslaan') : t('common.add', 'Toevoegen')}</button>
          {editIndex !== null && (
            <button type="button" onClick={cancelEdit}>{t('common.cancel', 'Annuleren')}</button>
          )}
        </div>
      </form>
